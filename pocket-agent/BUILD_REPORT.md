...
| M1 model + chat memory | PASS | 2 messages; final is an AI answer | yes | 3.11 |
| M2 durable multi-turn memory | PASS | recall on same thread_id; isolation across a new thread_id | yes | 7.0 |
| M3 tools + the cycle | PASS | agent -> tool(calculator=5929288761) -> final answer (the cycle) | yes | 3.61 |
| M4 streaming (v3 + fallback) | PASS | streamed a final answer via path='v3' |  | 2.31 |
| M5 human-approval gate (HITL) | PASS | interrupt pauses before write; resume=True writes, resume=False cancels | yes | 5.41 |
| M6 long-term Store | PASS | Store put/get/prefix-search works; graph compiles with store= |  | 0.0 |
| M7 time travel | PASS | 6 checkpoints enumerated; replayed from 1f176a03... |  | 4.44 |
...
| M11 DeltaChannel (beta) diff-based checkpoints | PASS | DeltaChannel reconstructs == full snapshot over 8 writes; checkpoint stores a sentinel (vs full list len 8); graph accumulated 5 steps |  | 0.0 |
| M12 Postgres persistence/store | PASS | PostgresSaver/PostgresStore compiled and preserved thread memory |  | 3.4 |
...
| ALT create_agent track | PASS | create_agent reached behavioral parity (6*7=42) |  | 2.17 |
