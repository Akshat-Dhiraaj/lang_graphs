...
| M1 model + chat memory | PASS | 2 messages; final is an AI answer | yes | 3.09 |
| M2 durable multi-turn memory | PASS | recall on same thread_id; isolation across a new thread_id | yes | 7.13 |
| M3 tools + the cycle | PASS | agent -> tool(calculator=5929288761) -> final answer (the cycle) | yes | 3.71 |
| M4 streaming (v3 + fallback) | PASS | streamed a final answer via path='v3' |  | 2.45 |
...
| M7 time travel | PASS | 6 checkpoints enumerated; replayed from 1f176a5a... |  | 4.6 |
...
| M9 middleware showcase + structured output | PASS | custom hooks OK; compiled; live structured_response present (mode=lmstudio) |  | 2.46 |
...
| M12 Postgres persistence/store | PASS | Postgres helpers import; live DB run skipped (set POCKET_POSTGRES_URI) |  | 0.08 |
