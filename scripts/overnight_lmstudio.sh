...
#   POCKET_CTX            context length        (default: 4096)
#   POCKET_GPU            GPU offload ratio      (default: max)   off|max|0..1
#   POCKET_PARALLEL       concurrent predictions (default: 1)
#   POCKET_TTL            unload idle seconds    (default: 28800)
...
#   qwen/qwen3.5-9b fits VRAM with full GPU offload at ctx=4096 and passed the
#   real tool-calling verifier; reliability matters more than max context here.
#   To try a bigger one in the morning, e.g.:
#     POCKET_MODEL=google/gemma-4-26b-a4b POCKET_GPU=0.6 bash scripts/overnight_lmstudio.sh
# =============================================================================
set -uo pipefail
...
if "$LMS" ps 2>/dev/null | grep -q "$MODEL"; then
  ...
  if "$LMS" load "$MODEL" --gpu "$GPU" -c "$CTX" --parallel "$PARALLEL" --ttl "$TTL" --identifier "$MODEL" -y >>"$LOG" 2>&1; then
    say "model loaded"
  else
...
say "---- launching scripts/build_pocket_agent.sh against LM Studio ----"
...
say "report : $REPO_ROOT/pocket-agent/BUILD_REPORT.md"
say "log    : $LOG"
