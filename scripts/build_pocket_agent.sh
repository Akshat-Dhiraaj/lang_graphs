...
# (see docs/plans/LangGraph_Foundations_Project_Plan.md).
#
...
#   - Implements milestones M0-M14 + the create_agent alternative track.
...
#
# Usage:
#   bash scripts/build_pocket_agent.sh            # full overnight build
#   bash scripts/build_pocket_agent.sh --check    # generate + syntax-check only (fast, no install)
#   bash scripts/build_pocket_agent.sh --clean    # wipe build dir first, then full build
#
# Config (env overrides):
#   POCKET_BUILD_DIR   target dir            (default: <repo root>/pocket-agent)
...
#   POCKET_SKIP_POSTGRES=1 to skip optional Postgres saver/store packages (M12 helper check)
#
...
if [ ! -d "$REPO_ROOT/.git" ] && [ "$(basename "$SCRIPT_DIR")" != "scripts" ]; then
  REPO_ROOT="$SCRIPT_DIR"
fi
BUILD_DIR="${POCKET_BUILD_DIR:-$REPO_ROOT/pocket-agent}"
...
__all__ = ["state", "tools", "model", "graph", "stream", "server_graph",
           "semantic", "delta_demo", "persistence", "cache_demo",
           "stream_projection"]
PY
...
PY
  ...
  # ---- pocket_agent/persistence.py  (Phase 5: optional Postgres persistence)
  cat > pocket_agent/persistence.py <<'PY'
...
PY
  ...
  cat > pocket_agent/cache_demo.py <<'PY'
...
PY
  ...
  # ---- pocket_agent/stream_projection.py  (Phase 5: custom v3 projection)
  cat > pocket_agent/stream_projection.py <<'PY'
...
PY
  ...
  # ---- tests/test_persistence.py  (Phase 5: optional Postgres helpers)
  cat > tests/test_persistence.py <<'PY'
import os
...
PY
  ...
  # ---- tests/test_stream_projection.py  (Phase 5: StreamTransformer)
  cat > tests/test_stream_projection.py <<'PY'
...
name = "pocket-agent"
version = "0.1.0"
description = "Foundations-first LangGraph build (generated by scripts/build_pocket_agent.sh)"
...
postgres = ["psycopg[binary,pool]", "langgraph-checkpoint-postgres"]
...
# POCKET_POSTGRES_URI=postgresql://postgres:postgres@localhost:5432/postgres?sslmode=disable
# POCKET_POSTGRES_SETUP=1      # optional: create/migrate LangGraph Postgres tables
...
MD
  ...
  say "generated: pocket_agent/{__init__,state,tools,model,graph,stream,cli,persistence,cache_demo,stream_projection}.py"
...
venv_and_install() {
  ...
  if [ "${POCKET_SKIP_POSTGRES:-}" = "1" ]; then
    say "POCKET_SKIP_POSTGRES=1 — skipping Postgres optional packages (M12 will skip live imports)"
  else
    install_grp postgres "psycopg[binary,pool]" langgraph-checkpoint-postgres
  fi
...
run_milestones() {
  phase "Verify milestones M0-M14 + alt track"
  cd "$BUILD_DIR" || return 1
  set +o pipefail
