...
# DeltaChannel is a *beta* API: present in langgraph >= 1.2.x, absent in 1.1.x.
# Guard the import so this module loads everywhere and M11 can SKIP (not fail)
# on a langgraph that doesn't ship it yet.
try:
    from langgraph.channels.delta import DeltaChannel
    from langgraph.channels.binop import BinaryOperatorAggregate
    HAVE_DELTACHANNEL = True
except Exception:
    DeltaChannel = None
    BinaryOperatorAggregate = None
    HAVE_DELTACHANNEL = False
...
def _delta_state():
    """Build the state schema lazily, so importing this module never requires
    DeltaChannel (the Annotated metadata is only evaluated when this is called)."""
...
def build_delta_demo_graph(steps: int = 5, *, checkpointer=None):
    ...
    Requires a langgraph that ships DeltaChannel (beta)."""
    if not HAVE_DELTACHANNEL:
        raise RuntimeError("DeltaChannel unavailable (beta; needs langgraph >= 1.2.x)")
    DeltaState = _delta_state()
